...
class Dataset(object):
    def __init__(self, opt, data_path=None):
        ...
        # #通过对训练集中最大实体的补充 寻找scores的维度
        # maxis = np.max(self.data['train'], axis=0)
        # self.n_entities = int(max(maxis[0], maxis[2]) + 1)

        #修改后从三个数据集中寻找最大值进行判定
        # 初始化一个变量来存储三个数据集中每一列的最大值
        max_train = np.max(self.data['train'], axis=0)
        max_valid = np.max(self.data['valid'], axis=0)
        max_test = np.max(self.data['test'], axis=0)

        # 计算三个数据集中每一列的最大值
        max_values = np.maximum(max_train, np.maximum(max_valid, max_test))

        # 更新实体数量
        self.n_entities = int(max(max_values[0], max_values[2]) + 1)
        self.n_predicates = int(max_values[1] + 1)
